...
def main(args):
    ...
    if pathlib.Path(f'{BASE_PATH_EXPS}/final_best_warmup.ckp').exists():
        ...
        model.load_state_dict(checkpoint['model_state_dict'])
        logger.info(f"Skipping warmup 64, model loaded from \
                    '{BASE_PATH_EXPS}/final_best_warmup.ckp'")
    else:
        logger.info(f"Running warmup 64,'{BASE_PATH_EXPS}/final_best_warmup.ckp' does not exist")
    ...
    if pathlib.Path(f'{BASE_PATH_EXPS}/final_best_search.ckp').exists():
        ...
        optimizer = torch.optim.SGD(net_parameters,
                                    lr=args.lr,
